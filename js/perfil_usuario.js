                    ...
                    if (xhr_perfil.readyState === 4 && xhr_perfil.status === 200) {
                        ...
                        if (lista_reviews.length > 0) {
                            lista_reviews.forEach((review) => {
                                agregar_juego_lista(review.id_review, review.id_juego, review.titulo, review.portada, review.puntuacion, review.critica, review.etiqueta_obra_maestra, review.etiqueta_sobrevalorado);
                            });
                        }
...
function modificar_juego() {
    ...
    let { generos_seleccionados, plataformas_seleccionadas } = desplegables_generos_plataformas(select_generos, contenedor_generos, select_Plataformas, contenedor_plataformas);
    ...
    formulario.addEventListener("submit", function(event) {
        ...
        let datos = `modo=modificar&titulo=${encodeURIComponent(titulo)}&portada=${encodeURIComponent(portada)}&desarrolladora=${encodeURIComponent(desarrolladora)}&plataforma[]=${plataformas_seleccion.join('&plataforma[]=')}&genero[]=${generos_seleccion.join('&genero[]=')}&lanzamiento=${encodeURIComponent(lanzamiento)}&duracion=${encodeURIComponent(duracion)}&descripcion=${encodeURIComponent(descripcion)}`;
...
function añadir_juego() {
    ...
    formulario.addEventListener("submit", function(event) {
        ...
        let datos = `modo=añadir&titulo=${encodeURIComponent(titulo)}&portada=${encodeURIComponent(portada)}&desarrolladora=${encodeURIComponent(desarrolladora)}&plataforma[]=${plataformas_seleccion.join('&plataforma[]=')}&genero[]=${generos_seleccion.join('&genero[]=')}&lanzamiento=${encodeURIComponent(lanzamiento)}&duracion=${encodeURIComponent(duracion)}&descripcion=${encodeURIComponent(descripcion)}`;
...
function desplegables_generos_plataformas(select_generos, contenedor_generos, select_Plataformas, contenedor_plataformas) {
    ...
    let generos = [
        "Arcade", "Action", "Adventure", "Sports", "Strategy", "Racing", "Figth", "Massively Multiplayer", "Platformer", "RPG", "Sandbox", "Shooter", "Simulation", "Survival", "Horror", "Puzzle", "Indie"
    ];

    let plataformas = [
        "Wii", "Wii U", "Nintendo Switch", "Nintendo 64", "GameCube", "Nintendo DS", "GameBoy Advance", "Nintendo 3DS", "PlayStation 5", "PlayStation 4", "PlayStation 3", "PlayStation 2", "PSX", "PSP", "PS Vita", "Xbox", "Xbox 360", "Xbox One", "Xbox Series S/X", "PC", "Dreamcast", "Android", "macOS", "Linux", "iOS", "Web"
    ];
...
function agregar_juego () {
    ...
    formulario.addEventListener("submit", function (event) {
        ...
        let consulta = `juego=${encodeURIComponent(juego_seleccionado)}&estrellas=${estrellas}&critica=${encodeURIComponent(critica)}&obra_maestra=${obra_maestra}&sobrevalorado=${sobrevalorado}`;
...
function eliminar_review() {
    ...
    btn_borrar.forEach(function (btn) {
        if (!btn.dataset.listener) { // Verificamos si el botón ya tiene el evento
