      ...
      if (xhr.readyState === 4 && xhr.status === 200) {
        ...
        xhrJuego.onreadystatechange = function () {
          if (xhrJuego.readyState === 4 && xhrJuego.status === 200) {
            ...
            if (rol === "invitado") {
              document.querySelector(".criticas").innerHTML =
                '<p>Debes <a class="registrate" href="registro_login.php?modo=registro">registrarte</a> e <a class="inicia_sesion" href="registro_login.php?modo=login">iniciar sesión</a> para poder ver las críticas de otros usuarios.</p>';
            } else {
  ...
  if (criticas_usuarios.length > 0) {
    ...
    criticas_usuarios.forEach((critica) => {
      ...
      btn_expandir.addEventListener("click", function () {
        ...
        this.textContent = critica_texto.classList.contains("expandir") ? "ver menos" : "ver más";
      });
    });
  } else {
